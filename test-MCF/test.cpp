...
static inline bool SolveMCF() {
 try {
  ...
  if (dMCFB) {
   FakeSolver* fs = dynamic_cast<FakeSolver*>((dMCFB->get_registered_solvers()).back());
   assert(fs);
...
int main( int argc , char **argv )
{
 ...
 switch (argc) {
  ...
  default:
   cerr << "Usage: " << argv[0] <<
        " <dmx file> [seed mode wchg #rounds #chng %chng optns]" << endl <<
        "       mode: 0 = only one, 1 = orig <- MILP, 2 = copy <- MILP" << endl <<
        "             +4 = abstract orig, +8 = abstract copy," << endl <<
        "             +16 = also change abstract representation" << endl <<
        "       wchg: what to change, coded bit-wise " << endl <<
        "             0 = cost, 1 = cap, 2 = dfct, 3 = o.arc, 4 = c.arc" << endl;
   return (1);
 }
 ...
 while (n_repeat--) {
  ...
  if ((wchg & 1) && (drand48() <= p_change)) {
   ...
   if (tochange == 1) {
    MCFBlock::CNumber newcst = c_min + drand48()*(c_max - c_min);
    ...
    for (MCFBlock::Index i = 0; i < tochange; i++)
     newcsts[i] = c_min + drand48()*(c_max - c_min);
