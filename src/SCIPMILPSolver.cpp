...
void SCIPMILPSolver::load_problem()
{
  ...
  {
    char *name = use_custom_names ? colname[i] : NULL;
    SCIP_Real collb = (lb[i] == -Inf<double>()) ? -SCIPinfinity(scip) : lb[i];
    SCIP_Real colub = (ub[i] == Inf<double>()) ? SCIPinfinity(scip) : ub[i];
    ...
    SCIP_VAR *var;
    SCIP_CALL_ABORT(SCIPcreateVarBasic(scip, &var, name, collb, colub, objective[i], SCIP_VARTYPE_CONTINUOUS));
  ...
  for (int i = 0; i < numrows; ++i)
  {
    ...
    char *name = use_custom_names ? rowname[i] : NULL;
    ...
    SCIP_CONS *cons;
    SCIP_CALL_ABORT(SCIPcreateConsBasicLinear(scip, &cons, name, 0, NULL, NULL, conslhs, consrhs));
...
int SCIPMILPSolver::compute(bool changedvars)
{
  ...
  {
    SCIP_CALL_ABORT(SCIPwriteOrigProblem(scip, output_file.c_str(), "mps", FALSE));
  }
...
{
  SCIP_CALL_ABORT(SCIPwriteOrigProblem(scip, filename.c_str(), "mps", FALSE));
}
...
void SCIPMILPSolver::var_modification(VariableMod *mod)
{
  ...
  */
  ...
  {
    SCIP_Bool fixed;
    SCIP_CALL_ABORT(SCIPfixVar(scip, vars[idx], var->get_value(), &infeas, &fixed));
    assert(fixed);
  }
  else if (var->is_integer())
  {
    if (var->is_unitary() && var->is_positive())
    {
      ...
      {
        SCIP_CALL_ABORT(SCIPchgVarType(scip, vars[idx], SCIP_VARTYPE_BINARY, &infeas));
      }
    }
    else if (oldtype != SCIP_VARTYPE_INTEGER)
    {
      SCIP_CALL_ABORT(SCIPchgVarType(scip, vars[idx], SCIP_VARTYPE_INTEGER, &infeas));
    }
  }
  else if (oldtype != SCIP_VARTYPE_CONTINUOUS)
  {
    SCIP_CALL_ABORT(SCIPchgVarType(scip, vars[idx], SCIP_VARTYPE_CONTINUOUS, &infeas));
  }
  assert(!infeas);

  SCIP_Real lb = var->get_lb() == -Inf<double>() ? -SCIPinfinity(scip) : var->get_lb();
  SCIP_Real ub = var->get_ub() == Inf<double>() ? SCIPinfinity(scip) : var->get_ub();
...
void SCIPMILPSolver::const_modification(ConstraintMod *mod)
{
  ...
  */
  ...
  case RowConstraintMod::eChgBTS:
    ...
    const_lhs = p_const->get_lhs() == -Inf<double>() ? -SCIPinfinity(scip) : p_const->get_lhs();
    const_rhs = p_const->get_rhs() == Inf<double>() ? SCIPinfinity(scip) : p_const->get_rhs();
...
void SCIPMILPSolver::add_dynamic_constraint(FRowConstraint *p_const)
{
  ...
  SCIP_Real const_lhs = p_const->get_lhs() == -Inf<double>() ? -SCIPinfinity(scip) : p_const->get_lhs();
  SCIP_Real const_rhs = p_const->get_rhs() == Inf<double>() ? SCIPinfinity(scip) : p_const->get_rhs();
  SCIP_CALL_ABORT(SCIPcreateConsBasicLinear(scip, &cons, NULL, 0, NULL, NULL, const_lhs, const_rhs));
...
void SCIPMILPSolver::add_dynamic_variable(ColVariable *p_var)
{
  ...
  lb =
      p_var->get_lb() == -Inf<double>() ? -SCIPinfinity(scip) : p_var->get_lb();
  ub = p_var->get_ub() == Inf<double>() ? SCIPinfinity(scip) : p_var->get_ub();
  ...
  assert(vars.size() == v_d_var_int.back().second + 1);
  ...
  SCIP_CALL_ABORT(SCIPcreateVarBasic(scip, &var, NULL, lb, ub, 0.0, vartype));
  vars.push_back(var);
  SCIP_CALL_ABORT(SCIPaddVar(scip, var));
