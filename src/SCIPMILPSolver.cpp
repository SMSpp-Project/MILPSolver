...
void SCIPMILPSolver::load_problem( void )
{
 ...
 if( is_qp ) {
  ...
  for( int i = 0 ; i < numcols ; ++i ) {
   ...
   SCIP_VARTYPE z_type;
   switch( xctype[ i ] ) {
    case( 'C' ): z_type = SCIP_VARTYPE_CONTINUOUS; break;
    case( 'B' ): z_type = SCIP_VARTYPE_BINARY;     break;
    case( 'I' ): z_type = SCIP_VARTYPE_INTEGER;    break;
    case( 'S' ):
    case( 'N' ):
    default:     SCIPABORT();
    }
 ...
 if( var->is_fixed() ) {  // fix the variable
  ...
  if( var->is_integer() && ( ! relax_int_vars ) ) {
   ...
   if( var->is_unitary() && var->is_positive() ) {
    ...
    if( oldtype != SCIP_VARTYPE_INTEGER )
     SCIP_CALL_ABORT( SCIPchgVarType( scip , vars[ idx ] ,
				      SCIP_VARTYPE_INTEGER , & infeas ) );
   }
  else
   if( oldtype != SCIP_VARTYPE_CONTINUOUS )
    SCIP_CALL_ABORT( SCIPchgVarType( scip , vars[ idx ] ,
